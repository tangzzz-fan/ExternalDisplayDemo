...
/// - `contentOffsetY`：内容容器随滚动 / 下拉产生的整体位移，**每帧都在变**。
///
/// 后者刻意做成 `item(at:)` 的参数而不是烘进 `fieldOrigin`：混在一起之后，
/// "内容滚了"和"布局变了"这两件事在调用点上就分不开了，而它们的更新频率差了两个数量级。
struct WaterfallFocus: Equatable, Sendable {
    ...
    ///
    /// - Parameter contentOffsetY: 内容容器当前的 y 偏移
    ///   （`ScrollMetrics.contentOffset(scroll:pull:)`，下拉为正、滚动为负）。
    ///
    ...
    func item(at point: CGPoint, contentOffsetY: CGFloat = 0) -> Int? {
        let local = CGPoint(
            x: point.x - fieldOrigin.x,
            y: point.y - fieldOrigin.y - contentOffsetY
        )
